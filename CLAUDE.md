...
- **`js/app/topbar.js`** — the top-bar shell (`initTopbar`): brand mark, active-robot chip (reflects build/drive via `state.mode`), and a light/dark theme toggle (`data-theme` on `<html>`, persisted to `localStorage`). Part of the Lab-Instrument identity (`--display` font, amber accent). The robot name is a seam for the M3 RobotDef work.
